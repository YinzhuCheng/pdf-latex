...
5. 设置 **并发数** 与 **最大重试次数**（默认：并发 2、重试 3 次）
6. 点 **1) 按页转写 + 剪裁出图**
7. 点 **2) 章节组织 + 生成 main.tex**
8. 点 **3) 导出 ZIP**（包含所有输出与 PNG）
...
## 断点续作：工作记录（Work Record）

为了解决长任务中断/网络波动，本工具支持把“当前已完成的转换结果”导出为 **工作记录 ZIP**，并在之后导入继续跑剩余页面。

- **导出工作记录**：点击“导出”（工作记录区）  
  - 会导出 `*_work_record.zip`，包含 `work_record.json`、`pages/page_###.json` 与已剪裁的 `p##_fig#.png`
- **导入工作记录**：先加载同一份 PDF，再选择工作记录 ZIP，点击“导入”  
  - 会校验 PDF 的 **SHA-256 指纹**（防止导错书）
  - 会把已完成页写入内存；再次点击“继续”或“按页转写”将 **自动跳过已完成页**

---

## 停止 / 继续 / 早停导出

- **停止**：中止当前运行（尽可能取消正在进行的请求），保留已完成的页结果
- **继续**：从断点继续（自动跳过已完成页）
- **早停导出**：不等跑完整个页码范围，立即中止并导出“工作记录 ZIP”（用于“到此为止就导出”）
